        ...
        while True:
            ...
            if cmd == "step":
                ...
                if reset_returns_info:
                    ...
                    p.send(obs)
            # if cmd == "reset":
            #     obs, state = env.reset(**data)  # Assuming this returns (obs, state)
            #     # Wrap state in info dict to comply with gym requirements
            #     info = {"state": state}
            #     pipe.send((obs, info))
            elif cmd == "close":
                p.send(env.close())
...
class SubprocEnvWorker(EnvWorker):
    ...
    def set_task_goal(self, goal_pose):
        self.parent_remote.send(["set_task_goal", goal_pose])
        return self.parent_remote.recv()
...
class BaseVectorEnv(object):
    ...
    def set_task_goal(self, goal_pose):
        """Set the goal pose for all environments."""
        self._assert_is_not_closed()
        for i in range(self.env_num):
            self.workers[i].set_task_goal(goal_pose[i])

    def step_once(self, actions):
        """Perform a single step in all environments."""
        self._assert_is_not_closed()
        for i in range(self.env_num):
            o, r, d,_, i = self.workers[i].step_once(actions[i])
        return o, r, d,_, i
